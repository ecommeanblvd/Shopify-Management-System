...
const HEADER = ['order', 'tracking', 'carrier', 'country', 'label_date', 'kind', 'state', 'reason', 'billed_vnd', 'delta_vnd', 'recovered_vnd', 'credit_note', 'approved_by', 'approved_at'];
...
export async function GET(): Promise<Response> {
  ...
  const out: CsvValue[][] = rows.map((r) => [
    ...
    carrierErrorKindLabel(r.kind), r.state === 'disputing' ? 'đang đòi' : r.state === 'credited' ? 'đã thu hồi' : r.state === 'accepted' ? 'chấp nhận chênh lệch' : 'đã duyệt', r.note, r.billedVnd, r.deltaVnd,
    r.recoveredVnd, r.creditNoteNumber,
