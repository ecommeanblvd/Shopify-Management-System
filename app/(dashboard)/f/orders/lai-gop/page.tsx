...
/** 'YYYY-MM' hợp lệ hay không (THUẦN). */
function thangHopLe(s: string | undefined | null): s is string {
  return !!s && /^\d{4}-(0[1-9]|1[0-2])$/.test(s);
}

/** Tháng cách `period` `soThang` tháng về trước, dạng 'YYYY-MM' (THUẦN). */
...
/** Danh sách các tháng từ `tu` đến `den` (bao gồm 2 đầu), tăng dần. Đảo lại nếu `tu` > `den`. Giới hạn 36 tháng để tránh truy vấn quá lớn. */
...
export default async function LaiGopPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  return (
    <div className="px-6 md:px-10 py-8 md:py-12 space-y-8">
      ...
      {canManage && <TiGiaForm hang={tiGiaTheoThang} />}
    </div>
  );
}
