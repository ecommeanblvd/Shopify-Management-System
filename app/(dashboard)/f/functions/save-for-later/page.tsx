...
import { ChevronLeft, Bookmark, Power, ExternalLink, Globe } from 'lucide-react';
...
import {
  listSaveForLaterStatusPerStore, setSaveForLaterEnabled,
...
import { SaveForLaterToggle } from '@/components/functions/SaveForLaterToggle';
import { CrossStoreActivityTable } from '@/components/functions/CrossStoreActivityTable';

export const dynamic = 'force-dynamic';

export default async function SaveForLaterAdminPage() {
  ...
  const [stores, crossStore] = await Promise.all([
    listSaveForLaterStatusPerStore(),
    getCrossStoreActivity('save-for-later'),
  ]);
  ...
  return (
    <div className="px-6 md:px-10 py-8 md:py-12 space-y-10">
      ...
      <header className="space-y-3">
        ...
        <h1 className="text-4xl md:text-5xl font-semibold tracking-tight">Save for later</h1>
              ...
              {stores.map((s) => (
                <li key={s.storeId} className="px-5 py-3 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {s.enabled && (
                        <Badge variant="secondary" className="h-4 text-[9px] uppercase tracking-wider gap-1 px-1.5">
                          <Power className="size-2.5 text-violet-500" />
                          active
                        </Badge>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    {s.enabled && (
                      <Link
                        href={`/f/functions/save-for-later/${s.storeId}`}
                    ...
                    <SaveForLaterToggle
                      ...
                      enabled={s.enabled}
                      canManage={canManage}
                      saveAction={setSaveForLaterEnabled}
                    />
                  </div>
                </li>
              ))}
      ...
      <Card>
        <CardContent className="p-0">
          ...
          <CrossStoreActivityTable
            rows={crossStore}
            adminPathPrefix="/f/functions/save-for-later"
            accentDot="text-violet-500"
          />
        </CardContent>
      </Card>
    </div>
