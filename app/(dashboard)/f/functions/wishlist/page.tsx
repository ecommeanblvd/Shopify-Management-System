...
import { ChevronLeft, Heart, Power, ExternalLink, Globe } from 'lucide-react';
import { auth } from '@/lib/auth/auth';
import { getRole } from '@/lib/auth/role';
import { hasPermission } from '@/lib/auth/rbac';
import { Card, CardContent } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { listWishlistStatusPerStore, setWishlistEnabled } from '@/features/functions/wishlist/admin-actions';
import { getCrossStoreActivity, rollupCrossStore } from '@/features/functions/cross-store';
import { WishlistToggle } from '@/components/functions/WishlistToggle';
...
export default async function WishlistAdminPage() {
  ...
  const [stores, crossStore] = await Promise.all([
    listWishlistStatusPerStore(),
    getCrossStoreActivity('wishlist'),
  ]);
    ...
    <div className="px-6 md:px-10 py-8 md:py-12 space-y-10">
      ...
      <header className="space-y-3">
        ...
        <p className="text-sm text-muted-foreground max-w-2xl">
          Customer-facing wishlist for connected Shopify stores. Toggle the
          per-store switch below to make the embed script + storefront API
          start serving requests for that shop. Per-store analytics live one
          level deeper.
        </p>
      </header>
              ...
              {stores.map((s) => (
                <li key={s.storeId} className="px-5 py-3 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {s.enabled && (
                        <Badge variant="secondary" className="h-4 text-[9px] uppercase tracking-wider gap-1 px-1.5">
                          <Power className="size-2.5 text-emerald-500" />
                          active
                        </Badge>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    {s.enabled && (
                      <Link
                        href={`/f/functions/wishlist/${s.storeId}`}
      ...
      <Card>
        <CardContent className="p-0">
          ...
          <CrossStoreActivityTable
            rows={crossStore}
            adminPathPrefix="/f/functions/wishlist"
            accentDot="text-rose-500"
          />
        </CardContent>
      </Card>
