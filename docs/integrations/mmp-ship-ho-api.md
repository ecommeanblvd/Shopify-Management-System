...
**Luật kỳ bảng kê (CEO chốt 22/09/2026, hai bên áp chung):** MMP xếp kỳ theo **ngày đơn được push sang MMP** — `occurredAt` của **lần push đầu tiên** `order.reconciled` (cước) / `order.duty_charged` (thuế-phí). SMS lập bản đối soát theo cùng mốc để khớp từng dòng. Ngày gửi (`shippedAt`), ngày hoá đơn FedEx (`invoiceDate`), ngày Đức chốt là số liệu nội bộ SMS — gửi kèm để tra cứu, KHÔNG dùng xếp kỳ. Bắn lại một đơn (đổi giá, tách duty) không đổi kỳ của đơn đó.
