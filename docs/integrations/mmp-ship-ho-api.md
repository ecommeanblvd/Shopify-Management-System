...
| | `order.cancelled` | Đã hủy. SMS gửi khi hãng xác nhận kiện CHƯA TỪNG GỬI (chỉ có sự kiện tạo nhãn). `data`: `{ reason: "khong_gui_hang", message, chargedVnd: 0, evidence? }` — brand không bị thu phí đơn này; MMP nên đóng đơn và bỏ giá đã báo. Gửi đúng một lần mỗi đơn. |
...
| Tài chính | `order.reconciled` | Hoá đơn carrier về → **giá cuối**. `data`: `{ finalChargedVnd, previousChargedVnd, deltaVnd, billedWeightKg, dutyVnd, totalWithDutyVnd, shippedAt, reconcileResolution? }` — MMP cập nhật giá cuối cho brand (thay giá dự kiến). Bắn khi: (a) bill KHỚP dự tính (tự động), hoặc (b) operator **chấp nhận sai lệch** (lỗi nội bộ) — khi đó `reconcileResolution = "internal_error"`. Đơn CÓ sai lệch mà CHƯA duyệt thì KHÔNG bắn (giá giữ nguyên dự kiến). **Từ ngày bật tách duty (báo trước 1 ngày): `finalChargedVnd` = cước, không gồm duty.** Sự kiện không có `dutyVnd` là bản cũ đã gộp duty. |
| Tài chính | `order.reconcile_pending` 🆕 | Bill carrier về CÓ sai lệch (chi phí thực ≠ dự tính) và **đang CHỜ operator duyệt**. MMP set đơn về trạng thái **"chờ đối soát"**, **GIỮ giá dự tính** (gỡ giá thực + nhãn "đã đối soát" nếu đã lỡ nhận trước đó); **KHÔNG** cập nhật giá cuối tới khi operator quyết định. `data: { estimatedCostVnd, billedCostVnd (cước, không duty), dutyVnd, deltaVnd }`. `billedCostVnd − estimatedCostVnd = deltaVnd`; duty đi riêng (tổng thật phải trả FedEx = `billedCostVnd + dutyVnd`). Sau đó SMS gửi `order.reconciled` (chấp nhận/kết luận claim) hoặc `order.claim_pending` (đi claim). |
| Tài chính | `order.claim_pending` 🆕 | Bill về CÓ sai lệch và operator quyết định **đòi carrier** (claim). MMP set đơn sang trạng thái **"đợi claim đơn vị vận chuyển"**; **KHÔNG** cập nhật giá cuối (giữ giá dự kiến tới khi claim xong). `data: { estimatedCostVnd, billedCostVnd (cước, không duty), dutyVnd, deltaVnd, reason (string\|null) }`. `billedCostVnd − estimatedCostVnd = deltaVnd`; duty đi riêng (tổng thật phải trả FedEx = `billedCostVnd + dutyVnd`). |
...
| Bảng kê | `statement.issued` / `statement.paid` | **Bản đối soát nội bộ** (phương án B, 21/09/2026): SMS phát hành bảng kê của mình → MMP so với bảng kê MMP theo `(mã đơn, loại)`, lệch thì báo; **không render cho brand**. `code = mmpRef = brandSlug`. `issued.data: { statementId, type: "freight"|"duty", periodStart, periodEnd, periodBasis: "first_push_at", orders: [{ code, mmpRef, brandReference, trackingNumber, shippedAt, amountVnd, fedexInvoiceNumber?, invoiceDate? }], orderCount, totalVnd }`. `paid.data: { statementId, type, paidAt }`. |

**Luật kỳ bảng kê (CEO chốt 22/09/2026, hai bên áp chung, từ kỳ 09/2026):** kỳ của một đơn = tháng của **lần push gần nhất tính đến lúc chốt kỳ** (`order.reconciled` cho cước, `order.duty_charged` cho thuế-phí). Kỳ đã khoá **không sửa**; đơn đổi số sau đó → **dòng điều chỉnh** (+/−) trong kỳ kế tiếp, cùng mã đơn, dẫn chiếu kỳ gốc (đúng tinh thần hoá đơn điều chỉnh NĐ 123/2020 Đ.19). `statement.issued` sẽ mang `orders[].adjustmentOf: { statementId, periodStart, periodEnd, previousAmountVnd }` cho dòng điều chỉnh (bổ sung khi SMS có bảng dòng). Kỳ 07/08/2026 Kalisa giữ như MMP đã khoá. `shippedAt`, `invoiceDate`: tra cứu, không xếp kỳ.
