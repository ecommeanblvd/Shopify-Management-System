...
- **Bỏ cột "Cost override / unit" (09/09/2026):** cột nhập tay này chính là "chi thật" thời chưa có bảng kê — CEO xác nhận trùng vai trò với cột Giá vốn thực. Dữ liệu: **0/15.563 dòng** từng dùng. Đã gỡ khỏi UI, khỏi `OrderLineDetail`, khỏi `UpdateOrderOverridesInput` và khỏi thứ tự ưu tiên giá vốn (nay chỉ còn: giá THỰC `order_line_cogs` → giá DỰ TÍNH `sku_costs`). Cột DB `shopify_order_lines.cost_override` giữ lại (đánh dấu @deprecated), không đọc/ghi. Nút "Sửa" trong modal giờ chỉ chỉnh cân nặng + ghi chú ship.
...
**Đợt 8 brand (09/09/2026, chiều)** — Cénes, Semi-Sense, Fair Lady, Armous, Jang Hana, Dang Phong Designer, Esmée, CEM. Không cần luật đọc mới; mọi kỳ 2026 Σ = tổng brand, khớp SKU 100 %, 0 không khớp. Ghi chú: Cénes USD + Tỷ giá Vietcombank (vendor Shopify "CÉNES" có dấu — tra vendor phải giữ dấu); Semi-Sense USD, không có dòng TỔNG ₫ (T2/T3 ghi "280.00$"), Σ = TT × tỉ giá; Fair Lady VND, không có dòng mục A/TỔNG (trừ T2), T4 có 6 chiếc #MTB offline (không khớp đơn nào để phân bổ); Armous và CEM có dòng "THUẾ (A*8%)" riêng → TT trước thuế; **Jang Hana một sheet cho 2 vendor** (JANG HANA / JANG HANA RTW) nhập dưới `jang-hana`, T8 trộn mục A USD (tỉ giá 25.880) + mục B VND cộng vào, T7 chưa hoàn tất (TT $0), tab "T3 thực bán" không có tiêu đề (bỏ); Dang Phong Designer T8 chưa hoàn tất; **Esmée** sheet ghi "ESMEE", brand chưa có trong `mmp_brands` → tạo dòng `esmee` (như Lemonrin) rồi nhập T7/T8 (VND, có dòng VAT riêng, không có dòng mục A). CEM có tab 2025 → ngoài phạm vi (không nhập). Nhập: Cénes 8 (T1/T2/T4/T7); Semi-Sense 8 (T2/T3/T5/T8); Fair Lady 7 + 6 #MTB; Armous 5 (T2–T5); Jang Hana 8 (T1/T4/T6/T8); Dang Phong 6 (T2/T4–T7); Esmée 6; CEM 6 (T2/T5/T6). Độ phủ 2026: Cénes 8/16, Semi-Sense 8/9, Fair Lady 7/8, Armous 5/8, Jang Hana (2 vendor) 8/10, Dang Phong 5/8, Esmée 6/8, CEM 5/6 — phần thiếu chủ yếu là dòng T8–T9 chờ kỳ sau và vài dòng PARTIALLY_REFUNDED brand không kê. Link "Laling" CEO gửi kèm là sheet LaLING đã nhập (LALIN 9 dòng vẫn chưa có sheet); **Lekieu** sheet vẫn trả HTTP 401 (chưa mở "ai có link đều xem được").

**De Theia + SoDope Club T8 (10/09/2026, CEO gửi lại sheet):** hai brand vừa hoàn tất tab T8 → nhập lại (kỳ cũ giữ nguyên số). De Theia T8: 14 dòng sheet, nhập 13, **1 không khớp**: #MBLVD30003 sheet ghi SKU "Detheia-5036402714547-M-NALM-PLA" nhưng đơn có 714530-M và 714547-S (brand ghép nhầm mã với size) — chờ brand sửa; độ phủ De Theia 89/113 (9 REFUNDED, 5 dòng T2–T4 đã giao chưa kê, 10 dòng T8–T9 chưa kê). SoDope Club T8: 16 dòng, khớp 100 %; độ phủ 31/45 (11 dòng T8–T9 chưa kê, 2 dòng T4/T5 chưa kê). Ước lại giá dự tính theo lịch sử mới: 2.861 SKU → 4.071 mức giá.

**Độ phủ MEAN BLVD 2026 (10/09):** 3.682 giá thực + 479 chỉ dự tính = 4.161/5.482 dòng (75,9 %). Trong 1.321 dòng chưa có giá: 600 dòng quà tặng/thiệp `MBLVD-…` (giá bán 0), 18 dòng Tip/Shipping fee (vendor trống) — không phải hàng; 220 dòng hàng brand bán sale (SKU `-Sale`, vendor MEAN BLVD, brand không kê) chờ CEO chốt nguồn giá; còn lại ~480 dòng brand chưa kê (phần lớn T8–T9 chờ kỳ sau, hoặc đã hoàn tiền).

**Đợt 5 brand nhỏ (10/09/2026)** — Lekieu (CEO đã mở quyền, hết 401), Emchic, Soramame, Duong Atelier, H2B à La Mode. Mọi kỳ Σ = TỔNG (A) ₫ của sheet, khớp SKU 100 %, 0 không khớp. Lekieu & Soramame USD + Tỷ giá Vietcombank (26.085 / 26.080 / 25.880; 26.108), CK 50 %, cột Note Lekieu chứa số nháp không khớp tỉ giá → bỏ, đúng luật; Emchic VND CK 25 % (tab không có cột Ngày → cột đầu); Duong Atelier VND CK 25 %, tab T6 chỉ có 1 dòng #MBLVD29754 (giá 62 tr) chưa điền CK/TT → bỏ qua, chờ brand; H2B VND CK 20 %, T3 là 1 chiếc **#MTB1330** (PO) → phân bổ cho #MBLVD29454 (T7, cùng SKU) 4.200.000đ. **Sửa lỗi phân bổ PO:** so vendor ↔ slug trong SQL bằng regexp bỏ `[^a-z0-9]` làm rơi chữ có dấu ("H2B à La Mode" → h2blamode ≠ h2balamode) → chuyển sang `chuanBrand` (bỏ dấu trước) trong `phan-bo-po-core.ts`, có test; chạy lại phân bổ PO cho 14 brand có PO, số không đổi (Huelleyrose 25,67 tr = 27,72 tr ÷ 1,08 sau quy về trước thuế). **LALIN:** CEO xác nhận LALIN = LaLing, nhưng sheet LaLING không có dòng nào cho 10 đơn SKU `LALIN-…` (#MBLVD27340, 28085, 28452, 29043, 29525, 29588, 29618, 29669, 29699, 29809) → brand chưa kê, cần ops hỏi. Độ phủ 2026: Lekieu 7/7, Emchic 5/6, Soramame 4/6, Duong Atelier 4/6, H2B 4/4. Ước lại giá dự tính: 2.885 SKU → 4.089 mức giá.
...
**Độ phủ giá vốn store MEAN BLVD 2026 sau đợt 8 brand chiều:** 3.653/5.476 dòng (66,7 %); 01–07/2026: 3.355/4.597 (73,0 %). 80 vendor đã nhập: 3.653/4.525; 37 vendor chưa có sheet: 951 dòng (MEAN BLVD tự sản xuất 828; vendor trống 18; LALIN 9; LEKIEU 7; còn lại ≤ 6 dòng/vendor: Soramame, Duong Atelier, Matt & Amy, Emchic 6, H2B à La Mode, Pesty, LEA'S by LeHa, Randomlist, Dear José, An Heritage 4, …).
