...
async function xuLy(recordId: string, dry: boolean): Promise<KetQuaNhanDong> {
  ...
  const daCo = await db.select({
    ...
    larkHop: schema.shipments.larkHop, originHub: schema.shipments.originHub, ngayDiDuKien: schema.shipments.ngayDiDuKien, donDiChung: schema.shipments.donDiChung, skuText: schema.shipments.skuText, pieces: schema.shipments.pieces,
