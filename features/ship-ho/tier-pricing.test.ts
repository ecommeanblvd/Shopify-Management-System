...
describe('SHIP_HO_TIERS (thang CEO 23/07: +30/+25/+20/+15/+10 trên base)', () => {
  it('5 bậc, rack 40% (chỉ trình bày CK), sàn volume ĐÚNG 10%', () => {
  ...
  it('markup hiệu dụng từng bậc ĐÚNG: 30 / 25 / 20 / 15 / 10', () => {
    const by = Object.fromEntries(SHIP_HO_TIERS.map((t) => [t.code, effectiveMarkupPercent(t.discountPct)]));
    expect(by.standard).toBeCloseTo(30, 6);
    expect(by.bronze).toBeCloseTo(25, 6);
