...
export function quote(snap: CarrierAccountSnapshot, input: QuoteInput): QuoteResult {
  ...
  return {
    ...
    breakdown: {
      ...
      dimWeightKg,
      chargeableWeightKg,
      // roundCost chứ KHÔNG phải Math.round: đơn vị tiền chi phí quyết định số
      // lẻ được giữ. VND không có thập phân nên kết quả y hệt như trước, còn
      // USD (Aramex) giữ đủ cent — làm tròn nguyên sẽ biến $25,17 thành $25,
      // mất 17 cent mỗi đơn và làm sai cả cột chi tiết trên màn đối soát.
      base: roundCost(base),
      fuel: roundCost(fuel),
      fuelPercent: fuelPct,
      peak: roundCost(peak),
      addons: roundCost(addons),
      addonReference: roundCost(addonReference),
      addonExcludedForCountry,
      remote: roundCost(remote),
      residential: roundCost(residential),
      perKg: roundCost(perKg),
      demand: roundCost(demand),
      countryFixed: roundCost(countryFixed),
      countryFixedReference: roundCost(countryFixedReference),
      perStep: roundCost(perStep),
      vatPercent: vatPct,
      vat: roundCost(vat),
      discountPercent: discountPct,
      discount: roundCost(discount),
      packaging: packagingCost,
      markup: roundCost(markup),
      subtotalBeforeMarkup: roundCost(subtotalBeforeMarkup),
      carrierCost,
      carrierCostDisplay,
      finalCost,
      finalDisplay,
