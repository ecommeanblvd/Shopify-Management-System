...
import { Coins, Globe2, Package } from 'lucide-react';
    ...
    <div className="space-y-10">
      ...
      <section className="space-y-3">
        <div className="text-xs uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
          <Coins className="size-3.5" /> {hasPak ? 'Package · hộp · cost (zone × tier)' : 'Rate matrix · cost (zone × tier)'}
        </div>
        <RateMatrix
          zones={matrixZones}
          tiers={tiers}
      ...
      {hasPak && (
        <section className="space-y-3">
          <div className="text-xs uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
            <Package className="size-3.5" /> PAK · cost (zone × bậc PAK)
          </div>
          <RateMatrix
            zones={matrixZones}
            tiers={pakTiers}
            initialCells={pakCells}
            costCurrency={costCurrency}
            canEdit={false}
            highlightZoneId={match?.zoneId ?? null}
          />
