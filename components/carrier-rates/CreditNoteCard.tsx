...
export function CreditNoteCard({ rows, tongThang }: { rows: CreditNoteRow[]; tongThang: Array<{ thang: string; tong: number; n: number }> }) {
  return (
    <Card><CardContent className="space-y-4 p-4">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          <div className="text-sm font-semibold">Credit note carrier</div>
          <p className="text-[11px] text-muted-foreground">
            Tiền thu hồi tính theo NGÀY TRÊN HOÁ ĐƠN, một hoá đơn VAT là một credit note. Đây cũng là số vào KPI Pillar 3.
          </p>
        </div>
        {tongThang.length > 0 && (
          <div className="text-xs text-muted-foreground">
            {tongThang.map((t) => `${t.thang}: ${tien(t.tong)} (${t.n})`).join(' · ')}
          </div>
        )}
      </div>
          ...
          <table className="w-full text-sm tabular-nums">
            <thead className="text-[11px] uppercase tracking-wide text-muted-foreground">
              <tr className="[&>th]:px-3 [&>th]:py-2 [&>th]:font-medium">
                <th className="text-left">Hoá đơn</th><th className="text-right">Ngày</th><th className="text-right">Số tiền thu hồi</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-t border-border/60 [&>td]:px-3 [&>td]:py-1.5">
                  ...
                  <td className="text-right font-medium text-emerald-600 dark:text-emerald-400">{tien(Math.abs(r.tongCong))}</td>
