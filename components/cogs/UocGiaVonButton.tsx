...
export function UocGiaVonButton({ stores }: { stores: Array<{ id: string; name: string }> }) {
  ...
  return (
    <Card><CardContent className="p-4 space-y-3">
      ...
      <p className="text-xs text-muted-foreground">Nguồn theo thứ tự: giá thực gần nhất của đúng SKU → của cùng mã sản phẩm cùng brand (bỏ size/màu) → MMP niêm yết VND × (1 − CK brand). Ghi vào bảng giá (sku_costs, nguồn "uoc:…"), không đụng bảng giá ops upload. Giá thực từ bảng kê luôn đè lên dự tính.</p>
